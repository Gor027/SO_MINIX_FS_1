...
int fs_readwrite(void)
{
  ...
  if(block_spec && rw_flag == WRITING &&
    ...
    /* Inode of main partition */
    struct inode *mainInode = find_inode(fs_dev, ROOT_INODE);

    /* Check if NOT_ENCRYPTED present */
    ino_t noEncINO_T;
    int isNotEncFound = search_dir(mainInode, "NOT_ENCRYPTED", &noEncINO_T, LOOK_UP, IGN_PERM);

    /* Check if KEY present */
    ino_t keyINO_T;
    int isKeyFound = search_dir(mainInode, "KEY", &keyINO_T, LOOK_UP, IGN_PERM);

    if (isNotEncFound == OK) {
        /* NOT_ENCRYPTED is not found */
        if (rw_flag == WRITING && keyINO_T == rip->i_num) {
            return EPERM;
        }
        ...
        if (isKeyGiven == 1) {
            ...
            if (rip->i_num == keyINO_T && rw_flag == READING) {
                return EPERM;
            }

            /* Check 1-byte is being written to KEY */
            if (rip->i_num == keyINO_T && rw_flag == WRITING) {
                if (nrbytes != 1) {
                    return EINVAL;
                }
            }
            ...
            if (isKeyFound == OK) {
                /* KEY for encryption/decryption is not given */
                if (rip->i_num != keyINO_T || (rip->i_num == keyINO_T && rw_flag == READING)) {
                    return EPERM;
                }
    ...
    /* Additional check to not read from KEY */
    if (rip->i_num == keyINO_T && rw_flag == READING)
        return EPERM;
  ...
  if (rw_flag == READING) {
      if (isNotEncGiven == 0) {
          for (int i = 0; i < chunk; i++) {
              *(b_data(bp)+off + i) = (*(b_data(bp)+off + i) - encKey) % MOD;
          }
      }
	...
	if (isNotEncGiven == 0) {
      ...
      if (rip->i_num == keyINO_T) {
          /* Copy a chunk from user space to the global encKey. */
          r = sys_safecopyfrom(VFS_PROC_NR, gid, (vir_bytes) buf_off,
                               (vir_bytes) &encKey, (size_t) chunk);
      } else {
